...
class _AllMitraPageState extends State<AllMitraPage> {
  ...
  void didChangeDependencies() {
    ...
    // Penanganan data argument yang lebih aman
    if (args != null && args.containsKey('dataList') && args['dataList'] is List) {
      dataList = List<Map<String, dynamic>>.from(args['dataList'] as List);
    } else {
      // Data placeholder untuk demonstrasi UI jika data kosong
      dataList = _placeholderData;
    }
  ...
  // Placeholder data untuk demonstrasi UI
  final List<Map<String, dynamic>> _placeholderData = [
    {'title': 'GoBox Pet Care Center', 'distance': 1.5, 'rating': 4.9, 'path_area': 'https://placehold.co/600x400/00AA13/FFFFFF?text=JP', 'duration': '30 min'},
    {'title': 'Gudang Penitipan Terpadu B', 'distance': 0.5, 'rating': 4.2, 'path_area': 'https://placehold.co/600x400/616161/FFFFFF?text=BDG', 'duration': '10 min'},
    {'title': 'Mitra Box Cepat Saji', 'distance': 5.2, 'rating': 4.7, 'path_area': 'https://placehold.co/600x400/FFC107/000000?text=SBY', 'duration': '1 jam'},
    {'title': 'Pusat Penyimpanan Logistik', 'distance': 2.1, 'rating': 3.9, 'path_area': 'https://placehold.co/600x400/4CAF50/FFFFFF?text=YK', 'duration': '45 min'},
    {'title': 'BoxGo Cabang Denpasar', 'distance': 0.9, 'rating': 5.0, 'path_area': 'https://placehold.co/600x400/2196F3/FFFFFF?text=BLI', 'duration': '15 min'},
  ];


  void filterData(String keyword) {
    // Selalu filter dari dataList asli
    ...
    // Terapkan sortasi yang sedang dipilih pada hasil filter
  ...
  void _applySort(List<Map<String, dynamic>> list, String filter) {
    if (filter == "Terdekat") {
      ...
      list.sort((a, b) => (a['distance'] as num).compareTo(b['distance'] as num));
  ...
  void applyFilter(String filter) {
    ...
    // 1. Filter berdasarkan keyword yang sedang ada
    List<Map<String, dynamic>> tempFiltered = dataList
        .where((item) =>
            item['title'].toString().toLowerCase().contains(keyword.toLowerCase()))
        .toList();

    // 2. Terapkan sortasi baru
    setState(() {
      filterSelected = filter;
      if (filter == "Semua") {
        // Terapkan hanya filter keyword, urutan default
            ...
            child: filteredList.isEmpty
                ? Center(
                    child: Text(
                      ...
                      ? "Belum ada mitra di sekitar Anda."
...
// Widget terpisah untuk Kartu Mitra (menggunakan konstanta warna)
